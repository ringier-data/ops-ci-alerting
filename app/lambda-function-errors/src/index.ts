...
export const handler: ScheduledHandler = async (event: ScheduledEvent) => {
  if (!cw) {
    cw = new CloudWatch({ region: process.env.AWS_REGION });
  }
  if (!slack) {
    /* istanbul ignore if */
    if (!process.env.SLACK_WEBHOOK_URL) {
      throw new Error('SLACK_WEBHOOK_URL must be defined');
    }
  ...
  if (scheduleInterval < 0 || Number.isNaN(scheduleInterval)) {
    throw new Error('Invalid RULE_INTERVAL_IN_MINUTES: ' + process.env.RULE_INTERVAL_IN_MINUTES);
  }
