...
export const main = async (
  ...
  scheduleRateIntervalInMinutes: number,
  ignoreFunctions: string[]
) => {
  if (event.source !== 'aws.events') {
    throw new Error('Got event.source "' + event.source + '". Only aws.events is supported.');
  }
  if (event['detail-type'] !== 'Scheduled Event') {
    throw new Error('Got event[detail-type] "' + event['detail-type'] + '". Only Scheduled Event is supported.');
  }
  ...
  const filteredFunctionsWhichDidErrors = functionsWhichDidError.filter((x) => !ignoreFunctions.includes(x.name));
  console.log(`Found ${filteredFunctionsWhichDidErrors.length} metrics after filtering which have errors > 0.`, {
    filteredFunctionsWhichDidErrors,
  });

  if (filteredFunctionsWhichDidErrors.length > 0) {
    await Promise.all(
      filteredFunctionsWhichDidErrors.map((x) =>
        slack.postMessage(getSlackBody(x.name, x.errors, endTime, scheduleRateIntervalInMinutes))
      )
    );
...
export const getSlackBody = (functionName: string, errorCount: number, endTime: Date, scheduleRateIntervalInMinutes: number) => {
  const textRealtime = `in the last ${scheduleRateIntervalInMinutes} minutes`;
  const textOlder = `between ${new Date(
    endTime.valueOf() - scheduleRateIntervalInMinutes * 60000
