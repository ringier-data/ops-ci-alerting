...
import { GetMetricDataInput, GetMetricDataOutput, ListMetricsInput, ListMetricsOutput, Metric, MetricDataResult } from 'aws-sdk/clients/cloudwatch'
...
describe('main', () => {
  ...
  const event = {
    ...
    "detail-type": "Scheduled Event" as "Scheduled Event",
    "source": "aws.events",
  ...
  it('should not send anything if listMetrics and getMetricData returns Metrics-property as paginated sets with all zeros ', async () => {
    ...
    mockGetMetricData(['metric-1', 'metric-2'], [[{functionName: 'metric-1', result: [0,0,0]}], [{functionName: 'metric-2', result: [0,0,0]}]])
    slackMock.setup(x => x.postMessage(It.isAny())).verifiable(Times.never())
  ...
  it('should send if listMetrics and getMetricData returns Metrics-property with > 0 errors on the 2nd page of results', async () => {
    ...
    mockListMetrics([[metric1, metric2]])
    mockGetMetricData(['metric-1', 'metric-2'], [[{functionName: 'metric-1', result: [0,0,0]}], [{functionName: 'metric-2', result: [0,0,1]}]]) // Note: 1 here 
  ...
  it('should send if listMetrics and getMetricData returns Metrics-property with > 0 errors on the 2nd page of results only for non-ignored functions', async () => {
    ...
    mockGetMetricData(['metric-1', 'metric-2', 'metric-3'], [
      [
  ...
  const mockListMetrics = (metricPages: Metric[][]) => {
    ...
    function buildListMetricsReturn(params: ListMetricsInput) {
      ...
      listMetricsReturn.setup(x => x.promise()).returns(() => Promise.resolve({
    ...
    cwMock.setup(x => x.listMetrics(It.is(y => {
        ...
        return true
      }))).returns((params: ListMetricsInput) => buildListMetricsReturn(params)).verifiable(Times.exactly(metricPages.length || 1))
  }
  ...
  const mockGetMetricData = (requestFunctionNames: string[], resultConfig: {functionName: string, result: number[]}[][]) => {
    ...
    function buildGetMetricDataReturn(params: GetMetricDataInput) {
      ...
      getMetricDataReturn.setup(x => x.promise()).returns(() => Promise.resolve({
    ...
    cwMock.setup(x => x.getMetricData(It.is(y => {
      expect(y).toMatchObject({
      ...
      requestFunctionNames.forEach(fnName => {
        expect(y.MetricDataQueries).toContainEqual(expect.objectContaining({
          MetricStat: {
            Metric: {
              Namespace: 'AWS/Lambda',
              MetricName: 'Errors',
      ...
      return true
    }))).returns((params: GetMetricDataInput) => buildGetMetricDataReturn(params)).verifiable(Times.exactly(resultConfig.length))
