...
const {handler: postRecords} = require('./index')
const testEventAlarm = require('./testEvent-EventBridge-alarm')
const testEventAwsBatch = require('./testEvent-EventBridge-awsbatch')
const nock = require('nock')

describe('Slack alarm lambda', () => {
  beforeAll(() => {
    process.env.AWS_REGION = 'fake-region'
    process.env.SLACK_HOOK_URL = faker.internet.url()
  ...
  describe('CW Alarm', () => {
    it('should post ALARM message to slack', async () => {
      ...
      nock(`https://logs.${process.env.AWS_REGION}.amazonaws.com/`, {"encodedQueryParams":true})
        ...
        .reply(200, {
          "metricFilters":[{"creationTime":1599125220237,"filterName":"sso-web-hooks-lambda-stg-WebHooksTaskExecuterMetricFilter-119CXVFX0EDOU",
      ...
      nock(`https://logs.${process.env.AWS_REGION}.amazonaws.com/`, {"encodedQueryParams":true})
        .post('/', {"logGroupName":"/aws/lambda/stg-sso-web-hooks-task-executor","filterPattern":"{$.level >= 50}","startTime":1599544153148,"endTime":1599544753148})
